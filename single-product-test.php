...
<div class="main_content">
    ...
    <section class="products_section">
        ...
        <div class="product_block">
					...
					<?php if (!empty($field_groups_3)): 
						foreach ($field_groups_3 as $fg):
							...
							if ($group_field && isset($group_field['sub_fields'][1])) {
								$group_value = get_field($group_field['name'], $product_id);
								if (is_array($group_value)) {
									$price_value = isset($group_value['price_options']['price_1']) ? $group_value['price_options']['price_1'] : '0';
									$clean_price = preg_replace('/[^\d]/', '', mb_substr($price_value, 5));
									$quantity = mb_substr($price_value, 0, 4);
					?>
						<div class="order_option third_type_option">
							<div class="title">
								<span class="decor_parameter"><?php echo esc_html(isset($group_value[$group_field['sub_fields'][0]['name']]) ? $group_value[$group_field['sub_fields'][0]['name']] : 'Гравировка'); ?></span>
								<span class="decor_quantity"><?php echo esc_html($quantity); ?></span>
							</div>
							<div class="price" id="decor_price" data-price="<?php echo esc_attr($clean_price); ?>">
								<?php echo number_format(floatval($clean_price), 0, '', ' ') . ' р.'; ?>
							</div>
						</div>
					<?php 
			...
			<?php foreach ($field_groups_3 as $fg):
				$fg_fields = (function_exists('acf_get_fields')) ? acf_get_fields($fg['ID']) : array();
				$group_fields = array();
				if (is_array($fg_fields)) {
					foreach ($fg_fields as $f) {
								...
								<?php foreach ($sub_fields as $sf):
									...
									<?php elseif (isset($sf['type']) && $sf['type'] === 'group' && isset($sf['sub_fields'])): ?>
										<div class="select_prices_options_container">
											<select name="decor_prices" class="prices_options">
												<?php foreach ($sf['sub_fields'] as $price_option):
													...
													if (!empty($price_val)):
												?>
													<option value="<?php echo esc_attr(mb_substr($price_val, 5)); ?>" 
															data-quantity="<?php echo esc_attr(mb_substr($price_val, 0, 4)); ?>"
															data-price="<?php echo esc_attr(preg_replace('/[^\d]/', '', mb_substr($price_val, 5))); ?>">
														<span class="decor_quantity_option"><?php echo esc_html(mb_substr($price_val, 0, 4)); ?></span>
...
jQuery(document).ready(function($) {
    ...
    function updateTotalPrice() {
        ...
        // Также проверяем все выбранные radio и select
        $('.option input:checked, .prices_options').each(function() {
            var $this = $(this);
            var price = 0;
            
            if ($this.hasClass('prices_options')) {
                price = parseInt($this.find('option:selected').data('price')) || 0;
            } else {
            ...
            $('.order_option .price').each(function() {
                var existingPrice = parseInt($(this).text().replace(/[^\d]/g, '')) || 0;
                if (existingPrice === price && price > 0) {
                    alreadyCounted = true;
                }
            });
            
            if (!alreadyCounted && price > 0) {
                totalPrice += price;
            }
        });
    ...
    // Универсальный обработчик для любых radio
    $(document).on('change', '.option_button', function() {
        var $this = $(this);
        var $option = $this.closest('.option');
    ...
    $(document).on('change', '.prices_options', function() {
        ...
        var $block = $select.closest('.options_block');
        var optionTitle = $block.find('.options_title').text();
        ...
        if ($orderOption.length) {
            $orderOption.find('.price').text(price.toLocaleString('ru-RU') + ' р.');
            $orderOption.find('.price').data('price', price);
            if ($orderOption.find('.decor_quantity').length && quantity) {
                $orderOption.find('.decor_quantity').text(quantity);
            }
        } else if (price > 0) {
            $('.order_options').prepend(
                '<div class="order_option">' +
                    '<div class="title">' + optionTitle + 
                        (quantity ? '<span class="decor_quantity">' + quantity + '</span>' : '') +
                    '</div>' +
                    '<div class="price" data-price="' + price + '">' + price.toLocaleString('ru-RU') + ' р.</div>' +
                '</div>'
            );
